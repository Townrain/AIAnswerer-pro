...
- 联网搜索设置新增「搜索模式」二选一：工具调用 / 预搜索注入（默认工具调用，可切回旧模式兼容不支持工具调用的模型）
- 工具模式自动回退：模型明确不支持函数调用时自动降级为预搜索注入模式
- WebSearchToolExecutor：抽取统一搜索执行器，预搜索与工具模式共用供应商层（Tavily/Zhipu/Bocha/Exa 等 10 家）
