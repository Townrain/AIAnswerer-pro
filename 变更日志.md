...
- 无工具模式答题请求启用 `response_format: json_object` 强制 JSON 输出（工具模式不传，兼容性稳妥）
- 设置页对不支持函数调用（无法联网搜索）的模型显示提示
- WebSearchToolExecutor：抽取统一搜索执行器，供全部搜索供应商复用（Tavily/Zhipu/Bocha/Exa 等 10 家）
- 新增真实 ExaMCP 供应商（MCP Streamable HTTP 客户端：initialize 会话握手 + tools/call JSON-RPC + SSE/JSON 双传输解析），消除静默 Tavily 兜底
- 录制防呆：停止录制时已获取的答案立即展示；录制中/处理中关闭弹窗仅隐藏窗口、保留数据；处理中跳过答案清理
- 设置页新增真并发测试入口（并发发 N 请求统计成功率），验证服务商真实并发能力；测试图改真实尺寸（720x1280 带文字）使结果有参考意义
- VLM 超时降级：120s 超时转失败并自动降级 OCR，不再静默丢失题目
...
- ChatMessage 支持 `tool_calls`/`tool_call_id`，content 可空；ChatRequest 新增 `tools`/`tool_choice`/`response_format` 参数
- 答案解析强化：工具伪文本剥离后保留其余文本（不再整段截断）；剥离 answer 值中的题号前缀（如「第2题：C」→「C」）；伪文本块后的 Markdown 答案不再被丢弃
- 录制模式：答题外层超时兜底（放宽至工具循环上限），进度分母统一为 max(截图数, 已识别题数)
- 录制提示词明确 answer/question 字段禁止包含题号前缀（根因治理）
...
- 修复连续两次录制时第二次结果不通知（start 复位通知幂等标志）
- 修复录制快速停止后立即重启时旧兜底协程抢占新会话结果（取消遗留兜底通知协程并复位处理中标志）
- 修复单题答题外层 60s 超时导致悬浮窗永久卡"获取答案中"（超时改为回调错误提示）
- 修复单题空答案渲染空白答案卡并清空剪贴板（空答案改错误提示）
- 修复录制停止后迟到的截图/文本仍被处理（丢弃迟到捕获，计数不再虚增）
- 修复一图多题时进度分母显示 x/1（统一为 max(截图数, 已识别题数)）
- 移除联网搜索设置保存时明文 API Key 日志
- 修复主页答题按钮在"选了模型但没填 API Key"半残状态下仍可点击（模型选择与 API Key 双重校验）
- ThemeManager 全 24 颜色字段校验 + 先写盘后改内存（导入失败不再吞异常导致重启后丢失）
- 隐身模式漏掉 D 详情窗无障碍旁路；Android 14 startForeground 显式传前台服务类型
- 悬浮窗窗口生命周期崩溃链与坐标修复
- 悬浮窗录制计数与拖拽手势修复
- 悬浮窗状态线程安全与代次机制
...
- 悬浮窗答案卡片与主按钮 16dp 缝隙贴合：卡片定位反向补偿按钮在窗口内的居中透明边距（pillEdgeMargin），视觉缝隙降为 0
