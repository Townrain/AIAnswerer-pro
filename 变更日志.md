...
### 新增
- 三窗口悬浮窗架构：将原来的单窗口拆分为 A（悬浮按钮）、B（快捷按键）、C（答案卡片）三个独立窗口，各自独立管理布局和触摸事件，间隙天然触摸穿透
...
### 变更
- FloatingWindowManager 完全重写为多窗口管理：新增 attachA/B/C、detachA/B/C、各窗口独立 updateLayoutA/B/C
- FloatingWindowService 大幅重构以支持三窗口协调与位置同步
...
- FloatingComponents：新增 cardWidthDp（360.dp）作为 Window C 固定宽度
