...
class DialogQueueTest {
    ...
    /**
     * Safely invoke [block], skipping the test via [Assume.assumeNoException]
     * if the Android runtime (MMKV/ConfigStorage) is unavailable.
     */
    private fun <T> safelyInvoke(block: () -> T): T {
        return try {
            block()
        } catch (e: Throwable) {
            Assume.assumeNoException(
                "DialogQueue requires Android runtime (MMKV) for this path; test skipped in JVM",
                e
            )
            @Suppress("UNREACHABLE_CODE")
            throw e // unreachable — assumeNoException throws
        }
    }
