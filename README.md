...
#### v1.4.2 (Bug修复)
* **修复 GitHub 链接跳转崩溃**
  - 修复"关于"页面点击 GitHub 链接时因缺少 `FLAG_ACTIVITY_NEW_TASK` 导致 `AndroidRuntimeException` 崩溃的问题
* **修复浮窗空闲时占据屏幕空间**
  - 修复空闲状态下浮窗仍保持 200dp 固定高度，多余透明区域拦截下层应用触摸的问题
  - 空闲时浮窗高度自动缩小至仅包裹悬浮按钮（约 72dp），触摸可穿透至下层应用
  - 显示答案卡片或展开快捷按钮时自动恢复正常高度

#### v1.4.1 (快捷按钮布局 & 设置优化)
* **快捷按钮布局模式**
  - 新增快捷按钮排列方式设置：弧形排列 / 横向排列，可在设置页随时切换
  - 弧形排列：半圆环绕主按钮，展开时窗口动态扩展（原有方式）
  - 横向排列：水平排列在主按钮旁，与原始设计一致，展开时窗口不变
  - 横向排列支持 Q 弹展开/收起动画（expandHorizontally + scaleIn 弹簧效果）
  - 拖拽主按钮时自动收起快捷按钮
* **深色模式适配**
  - 修复主页状态卡片图标在深色模式下颜色不适配的问题
  - 未激活状态图标改为 #FCA731 橙色，深色模式下对比度更好
* **设置页优化**
  - 合并并发数设置和连接测试为同一个卡片，中间用分隔线区分
  - 界面更紧凑，减少卡片数量
